...
class TestCLIRenderer(unittest.TestCase):
    ...
    def test_get_piece_char(self):
        ...
        self.assertEqual(self.renderer._get_piece_char("white"), "W")
        self.assertEqual(self.renderer._get_piece_char("black"), "B")

    def test_get_owner_and_count(self):
        """Prueba que el helper de 'draw' usa la API de Tablero (DIP)."""
        owner, count = self.renderer._get_owner_and_count(self.tablero, 0)
        self.assertEqual(owner, "black")
        self.assertEqual(count, 2)

        owner, count = self.renderer._get_owner_and_count(self.tablero, 23)
        self.assertEqual(owner, "white")
        self.assertEqual(count, 2)

        owner, count = self.renderer._get_owner_and_count(self.tablero, 1)
        self.assertEqual(owner, None)
        self.assertEqual(count, 0)

    def test_get_drawing_grid(self):
        """Prueba la lógica principal de 'draw' (movida desde Tablero)."""
        grid = self.renderer._get_drawing_grid(self.tablero)
    ...
    def test_mostrar_mensajes(self, mock_print):
        ...
        self.assertIn("-" * self.renderer.BOARD_WIDTH, mock_print.call_args_list[-1][0][0])
...
class TestCLIController(unittest.TestCase):
    ...
    def test_parsear_input_movimientos_validos(self):
        ...
        self.assertEqual(self.controller._parsear_input("23 20", "W"), (23, 20))
        self.assertEqual(self.controller._parsear_input("BAR 21", "W"), (24, 21))
        self.assertEqual(self.controller._parsear_input("BAR 3", "B"), (-1, 3))
        self.assertEqual(self.controller._parsear_input("2 OFF", "W"), (2, -1))
        self.assertEqual(self.controller._parsear_input("22 OFF", "B"), (22, 25))
        self.assertEqual(self.controller._parsear_input("bar 5", "B"), (-1, 5))
        self.assertEqual(self.controller._parsear_input("3 off", "W"), (3, -1))
